...
<script>
import {
  getAgentAgentUserList,
  getAgentUserInfo,
  getAgentUserResetpw,
  getAgentUserUseAccount,
  getAgentUserStopAccount
} from '@/api/account'
...
export default {
  ...
  methods: {
    getTableList() {
      ...
      getAgentAgentUserList(params).then(res => {
        if (res.code === 0) {
    ...
    dialogPwdData(userId, repass) {
      ...
      getAgentUserResetpw(userId, repass).then(res => {
        console.log(res)
        this.$message({
          type: 'success',
          message: '重设密码成功!'
    ...
    useAccount(userIds) {
      ...
      this.$confirm('此操作将启用选择用户, 是否继续?', '提示', {
        ...
        getAgentUserUseAccount(userIds).then(res => {
          if (res.code === 0) {
            const status = res.data.opreaState
            if (status) {
              this.$message({
    ...
    stopAccount(userIds) {
      ...
      this.$confirm('此操作将禁用选择用户, 是否继续?', '提示', {
        ...
        getAgentUserStopAccount(userIds).then(res => {
          if (res.code === 0) {
            const status = res.data.opreaState
            if (status) {
              this.$message({
    ...
    resetPaswd(userIds) {
      ...
      this.PaswdDialog = !this.PaswdDialog
      getAgentUserInfo(userIds).then(res => {
